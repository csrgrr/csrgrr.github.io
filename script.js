...
function refreshButtons(programWindows) {
  ...
  $.each(programWindows, function (name) {
    ...
    let button = $(
      '<button class="program" id="' + idName + '">' + name + "</button>"
    );
    leftFooterContainer.append(button);

    button.click(function () {
      if (
        programWindows[name].is(":visible") &&
        programWindows[name].find(".window-header").css("background-color") ===
          "rgb(0, 0, 255)"
      ) {
